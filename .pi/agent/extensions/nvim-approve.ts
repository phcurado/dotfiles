...
async function reviewInNvim(
  ...
  await writeFile(
    ...
    [
      ...
      "if has('nvim')",
      "  windo nnoremap <buffer> <silent> <nowait> g? :lua PiApprovalHelp()<CR>",
      "endif",
      "windo setlocal foldmethod=diff foldlevel=0",
      "wincmd h",
      "setlocal readonly nomodifiable",
      "let &l:statusline = ' ORIGINAL  %{g:pi_display_path} %=g? help  Space+a/ga approve  Space+q/gq cancel'",
      "if exists('&winbar')",
      "  let &l:winbar = 'ORIGINAL  %{g:pi_display_path}'",
      "endif",
      "wincmd l",
      "setlocal noreadonly modifiable modified",
      "let &l:statusline = ' PROPOSED (editable)  %{g:pi_display_path} %=g? help  Space+a/ga approve  Space+q/gq cancel'",
      "if exists('&winbar')",
